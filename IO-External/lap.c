...

{
  ...
  free = (row*)malloc(dim*sizeof(row));       // list of unassigned rows.
  collist = (col*)malloc(dim*sizeof(col));    // list of columns to be scanned in various ways.
  matches = (col*)malloc(dim*sizeof(col));    // counts how many times a row could be assigned.
  d =  (cost*)malloc(dim*sizeof(cost));         // 'cost-distance' in augmenting path calculation.
  pred = (row*)malloc (dim*sizeof(row));       // row-predecessor of column in augmenting/alternating path.
